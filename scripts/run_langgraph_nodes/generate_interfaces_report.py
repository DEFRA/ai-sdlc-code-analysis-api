#!/usr/bin/env python
"""
Script to test the generate_interfaces_report node in isolation.

This script creates a sample CodeAnalysisState with test data and passes it to the
generate_interfaces_report function to test it independently of the full LangGraph.
"""
...
from app.code_analysis.agents.nodes.interfaces_report import (  # noqa: E402
    generate_interfaces_report,  # noqa: E402
)
...
def create_test_state() -> CodeAnalysisState:
    ...
    try:
        ...
        logger.info("Successfully loaded test state for interfaces report generation")
        return state
...
async def main() -> None:
    ...
    logger.info("Starting generate_interfaces_report node test")
    ...
    logger.info("Calling generate_interfaces_report node...")
    try:
        # The function returns a CodeAnalysisState, but we need to await it since it's async
        result = await generate_interfaces_report(state)

        logger.info("Node execution completed successfully")

        # Print the interfaces report
        print("\n=== Interfaces Report ===\n")
        print(result.report_sections.interfaces)
