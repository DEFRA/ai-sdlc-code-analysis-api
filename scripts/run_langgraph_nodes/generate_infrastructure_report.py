#!/usr/bin/env python
"""
Script to test the generate_infrastructure_report node in isolation.

This script creates a sample CodeAnalysisState with test data and passes it to the
generate_infrastructure_report function to test it independently of the full LangGraph.
"""
...
from app.code_analysis.agents.nodes.infrastructure_report import (  # noqa: E402
    generate_infrastructure_report,  # noqa: E402
)
...
def create_test_state() -> CodeAnalysisState:
    ...
    try:
        ...
        logger.info(
            "Successfully loaded test state for infrastructure report generation"
        )
        return state
...
async def main() -> None:
    ...
    logger.info("Starting generate_infrastructure_report node test")
    ...
    logger.info("Calling generate_infrastructure_report node...")
    try:
        # The function returns a CodeAnalysisState, but we need to await it since it's async
        result = await generate_infrastructure_report(state)

        logger.info("Node execution completed successfully")

        # Print the infrastructure report
        print("\n=== Infrastructure Report ===\n")
        print(result.report_sections.infrastructure)
        ...
        save_output(
            ...
            script_dir,
            "infrastructure_report_result.json",
        )
