#!/usr/bin/env python
"""
Script to test the generate_business_logic_report node in isolation.

This script creates a sample CodeAnalysisState with test data and passes it to the
generate_business_logic_report function to test it independently of the full LangGraph.
"""
...
from app.code_analysis.agents.nodes.business_logic_report import (  # noqa: E402
    generate_business_logic_report,  # noqa: E402
)
...
def create_test_state() -> CodeAnalysisState:
    ...
    try:
        ...
        logger.info(
            "Successfully loaded test state for business logic report generation"
        )
        return state
...
async def main() -> None:
    ...
    logger.info("Starting generate_business_logic_report node test")
    ...
    logger.info("Calling generate_business_logic_report node...")
    try:
        # The function returns a CodeAnalysisState, but we need to await it since it's async
        result = await generate_business_logic_report(state)

        logger.info("Node execution completed successfully")

        # Print the business logic report
        print("\n=== Business Logic Report ===\n")
        print(result.report_sections.business_logic)
        ...
        save_output(
            ...
            script_dir,
            "business_logic_report_result.json",
        )
