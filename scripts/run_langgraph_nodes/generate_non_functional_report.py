#!/usr/bin/env python
"""
Script to test the generate_non_functional_report node in isolation.

This script creates a sample CodeAnalysisState with test data and passes it to the
generate_non_functional_report function to test it independently of the full LangGraph.
"""
...
from app.code_analysis.agents.nodes.non_functional_report import (  # noqa: E402
    generate_non_functional_report,  # noqa: E402
)
...
def create_test_state() -> CodeAnalysisState:
    ...
    try:
        ...
        logger.info(
            "Successfully loaded test state for non-functional report generation"
        )
        return state
...
async def main() -> None:
    ...
    logger.info("Starting generate_non_functional_report node test")
    ...
    logger.info("Calling generate_non_functional_report node...")
    try:
        # The function returns a CodeAnalysisState, but we need to await it since it's async
        result = await generate_non_functional_report(state)

        logger.info("Node execution completed successfully")

        # Print the non-functional report
        print("\n=== Non-Functional Report ===\n")
        print(result.report_sections.non_functional)
        ...
        save_output(
            ...
            script_dir,
            "non_functional_report_result.json",
        )
