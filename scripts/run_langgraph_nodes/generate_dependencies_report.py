#!/usr/bin/env python
"""
Script to test the generate_dependencies_report node in isolation.

This script creates a sample CodeAnalysisState with test data and passes it to the
generate_dependencies_report function to test it independently of the full LangGraph.
"""
...
from app.code_analysis.agents.nodes.dependencies_report import (  # noqa: E402
    generate_dependencies_report,  # noqa: E402
)
...
def create_test_state() -> CodeAnalysisState:
    ...
    try:
        ...
        logger.info("Successfully loaded test state for dependencies report generation")
        return state
...
async def main() -> None:
    ...
    logger.info("Starting generate_dependencies_report node test")
    ...
    logger.info("Calling generate_dependencies_report node...")
    try:
        # The function returns a CodeAnalysisState, but we need to await it since it's async
        result = await generate_dependencies_report(state)

        logger.info("Node execution completed successfully")

        # Print the dependencies report
        print("\n=== Dependencies Report ===\n")
        print(result.report_sections.dependencies)
        ...
        save_output(
            ...
            script_dir,
            "dependencies_report_result.json",
        )
