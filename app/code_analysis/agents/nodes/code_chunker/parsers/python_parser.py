...
class PythonParser(BaseLanguageParser):
        ...
        try:
            ...
            for capture in method_captures:
                ...
                if capture_name == "class.name":
                    ...
                    name = content[node.start_byte : node.end_byte]
                    self.logger.debug(
                        "Found method: %s in class: %s", name, current_class
                    )
    ...
    def _extract_classes(self, content: str, root_node, _, elements: dict[str, Any]):
        ...
        try:
            # First extract top-level classes
            class_nodes = self._query_nodes(root_node, "class_definition")
            ...
            # Now try to find nested classes
            for class_node in class_nodes:
                body_node = self._find_child(class_node, "block")
                if body_node:
                    # Look for class definitions within this block
                    nested_class_nodes = self._query_nodes(
                        body_node, "class_definition"
                    )
                ...
                try:
                    ...
                    if parent and parent.type == "expression_statement":
                        ...
                        if grandparent and grandparent.type in ("block", "module"):
                            # Check if it's the first child or right after function/class def
                            is_docstring = False

                            # If it's the first statement in a module, it's a module docstring
                            if (
                                grandparent.type == "module"
                                and parent == grandparent.children[0]
                            ):
                                is_docstring = True

                            # If it's in a block, check if preceded by def/class
                            elif grandparent.type == "block":
                                # Get index of the expression statement
                                for i, child in enumerate(grandparent.children):
                                    if child == parent and i == 0:
