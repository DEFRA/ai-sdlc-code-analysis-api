...
class ScalaParser(BaseParser):
    ...
    def __init__(self):
        ...
        self.method_pattern = (
            ...
            r")??\s*=?"
            r"|val\s+(\w+)(?:\s*:\s*" + self.effect_type_pattern + r")??\s*=?"  # Val definitions
            r"|var\s+(\w+)(?:\s*:\s*" + self.effect_type_pattern + r")??\s*=?"  # Var definitions
            r"|for\s*\{([^}]*)\}\s*yield\s*\{"  # For comprehensions
            r"|(?:map|flatMap|filter|fold|reduce|evalMap|parTraverse|parSequence)\s*\{"  # Common FP operations
            r")"
        )
        
        # Additional patterns for functional constructs
        self.for_comp_pattern = r"for\s*\{([^}]*)\}\s*yield\s*\{([^}]*)\}"
        self.type_class_pattern = r"implicit\s+(?:class|object)\s+(\w+)(?:\[.*?\])?\s*(?:extends|\(|{|$)"
        self.implicit_pattern = r"implicit\s+(?:val|def)\s+(\w+)(?:\[.*?\])?\s*(?:\(.*?\))?(?:\s*:\s*\w+(?:\[.*?\])?)??\s*=?"
            ...
            for match in re.finditer(pattern, content, re.MULTILINE):
                ...
                if class_info:
                    class_data = {
                        ...
                        "for_comprehensions": self._extract_for_comprehensions(class_info["body"]),
                    ...
                    type_info = self._extract_type_info(content[start_pos:class_info["end_pos"]])
        ...
        for match in re.finditer(self.method_pattern, class_content, re.MULTILINE):
            ...
            if method_info:
                method_data = {
                    ...
                    "is_for_comprehension": bool(re.match(self.for_comp_pattern, class_content[start_pos:]))
                }
                
                # Extract type signatures
                type_sig = self._extract_type_signature(class_content[start_pos:method_info["end_pos"]])
                if type_sig:
        ...
        for match in re.finditer(self.implicit_pattern, content, re.MULTILINE):
            ...
            if implicit_info:
                implicits.append({
                    "name": match.group(1),
                    "start_line": self._get_line_number(content, start_pos),
                    "end_line": self._get_line_number(content, implicit_info["end_pos"]),
    ...
    def _extract_type_signature(self, content: str) -> Optional[str]:
        ...
        type_sig_pattern = r":\s*((?:\w+(?:\[.*?\])?(?:\s*=>\s*\w+(?:\[.*?\])?)*))(?:\s*=|$)"
        ...
        for match in re.finditer(self.effect_construct_pattern, content, re.MULTILINE):
            ...
            if construct_info:
                patterns.append({
                    ...
                    "start_line": self._get_line_number(content, start_pos),
                    "end_line": self._get_line_number(content, construct_info["end_pos"]),
        ...
        for match in re.finditer(self.error_handling_pattern, content, re.MULTILINE):
            ...
            if handler_info:
                handlers.append({
                    ...
                    "start_line": self._get_line_number(content, start_pos),
                    "end_line": self._get_line_number(content, handler_info["end_pos"]),
    ...
    def _extract_type_info(self, content: str) -> Dict[str, Any]:
        ...
        # Extract type class constraints
        ...
        if match:
            type_info["type_class_constraints"] = [
                constraint.strip() 
                for constraint in match.group(1).split(",")
            ]
